...
code pour discriminer les BrS symptomatiques et non symptomatiques 
...
Dans cette base, les 22 premières lignes sont des patients contrôles et les 21 patients suivants sont pathologiques. 

```{r}
data_brs <- read.csv("/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/desc_BrS_AS_VS_S.csv" , sep=";",header = TRUE)
#data_brs <- read.csv("brs_nom.csv" , sep=";",header = TRUE)
```
...
```{r}
indNA <- which(is.na(data_brs),arr.ind=TRUE)[,1]
data_brs <- data_brs[-indNA,]
rownames(data_brs) <- NULL
```

```{r}
y_brs <- rep(NA, nrow(data_brs))

for (i in 1:nrow(data_brs)){
    if (i < 23) {
        y_brs[i] <- 0
    }
    else {
...
```{r}
for(i in 1:1680){
  colnames(data_brs)[i] <- paste(Name_desc[1,i],Name_desc[2,i],Name_desc[3,i],Name_desc[4,i],sep = "_")
}
```

```{r}
side <- as.data.frame(setNames(replicate(1680,numeric(0), simplify = F),colnames(data_brs) ))
side [1,] <- Name_desc[5,]
```
...
```{r}
#base pour l'ACP
data_b <- data.frame(y_brs,data_brs)
```
...
```{r}
et_f <- sort(apply(data_brs,2,sd), decreasing = TRUE)
```
...
```{r}
#acp normée 
acp_norme <- PCA(data_b, scale.unit=TRUE, quali.sup =1, graph=F)
```
...
```{r}
#tree_donnees_entieres_brs <- hclustvar(X.quanti = data_brs)
```

```{r}
#save(tree_donnees_entieres_brs, file = "/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/tree_donnees_entieres_brs.RData")
```

```{r}
load("/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/tree_donnees_entieres_brs.RData")
```


```{r}
cut_VC_brs <- cutreevar(tree_donnees_entieres_brs,25)
var_synth_brs <- cut_VC_brs$scores
save(var_synth_brs,file="/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/var_synth_brs.RData")
load("var_synth_brs.RData")
```
...
err_elast <- rep(NA,B) 
mat <- matrix(NA,dim(data_brs)[1],B)
tvp <- rep(NA,B)
preci <- rep(NA,B)
...
mat_rf <- matrix(NA,dim(data_brs)[1],B)
tvp_rf <- rep(NA,B)
preci_rf <- rep(NA,B)
...
mat_elast_synth <- matrix(NA,dim(data_brs)[1],B)
tvp_elast_synth <- rep(NA,B)
preci_elast_synth <- rep(NA,B)
choix_var_elast_synth <- matrix(NA,n,26)
nb_var_elast_synth <- rep(NA,n)

err_covvsurf_synth <- rep(NA,B)
mat_rf_synth <- matrix(NA,dim(data_brs)[1],B)
...
mat_elast_gram <- matrix(NA,dim(data_brs)[1],B)
tvp_elast_gram <- rep(NA,B)
preci_elast_gram <- rep(NA,B)
choix_var_elast_gram <- matrix(NA,n,dim(data_brs)[1]+1)
nb_var_elast_gram <- rep(NA,n)

err_covvsurf_gram <- rep(NA,B)
mat_rf_gram <- matrix(NA,dim(data_brs)[1],B)
...
mat_gram_no <- matrix(NA,dim(data_brs)[1],B)
...
sain_d <- data_brs[1:22,]
patho_d <- data_brs[23:dim(data_brs)[1],]

sain_d_synth <- var_synth_brs[1:22,]
patho_d_synth <- var_synth_brs[23:dim(data_brs)[1],]


y_sain <- y_brs[1:22]
y_patho <- y_brs[23:dim(data_brs)[1]]
...
for (b in 1:B)
{
  ...
  sain <- sample(rep(1:n_folds, length.out = 22))
  patho <- sample(rep(1:n_folds, length.out = dim(data_brs)[1]-22))
  ...
  for (alpha_ind in 1:length(alpha)) {
    ...
    err_alpha[alpha_ind] <- sum(c(pred_alpha_s,pred_alpha_p)!=y_brs[c(test_i_s,test_i_p+22)])/length(y_brs[c(test_i_s,test_i_p+22)])
  ...
  if(length(var_selected) == 1){
    ...
    colnames(X_train_selected) <- colnames(data_brs)[var_selected]
    colnames(X_test_selected_s) <- colnames(data_brs)[var_selected]
    colnames(X_test_selected_p) <- colnames(data_brs)[var_selected]
  }
  ...
  for (s in 1:ncol(data_brs)){
    for (t in 1:length(var_selected)){
    if (colnames(data_brs[s]) == colnames(X_train_selected[t])){
  ...
  for (alpha_ind in 1:length(alpha)) {
    ...
    err_alpha_synth[alpha_ind] <- sum(c(pred_alpha_s_synth,pred_alpha_p_synth)!=y_brs[c(test_i_s,test_i_p+22)])/length(y_brs[c(test_i_s,test_i_p+22)])
  ...
  if(length(var_selected_synth) == 1){
    ...
    colnames(X_train_selected_synth) <- colnames(data_brs)[var_selected_synth]
    colnames(X_test_selected_s_synth) <- colnames(data_brs)[var_selected_synth]
    colnames(X_test_selected_p_synth) <- colnames(data_brs)[var_selected_synth]
  }
  ...
  for (s in 1:ncol(data_brs)){
    ...
    if (colnames(data_brs[s]) == colnames(X_train_selected_synth[t])){
  ...
  # #données gram schmidt
  Ytrain_g <- t(t(Ytrain))
  index = gram(Xtrain,Ytrain_g,dim(data_brs)[1])
  var_l_1se <- index[1:dim(data_brs)[1]]
  c <- colnames(data_brs)[var_l_1se]
  indices_1se <- which(colnames(data_brs) %in% c)
  ...
  for (alpha_ind in 1:length(alpha)) {
    ...
    err_alpha_gram[alpha_ind] <- sum(c(pred_alpha_s_gram,pred_alpha_p_gram)!=y_brs[c(test_i_s,test_i_p+22)])/length(y_brs[c(test_i_s,test_i_p+22)])
  ...
  if(length(var_selected_gram) == 1){
    ...
    colnames(X_train_selected_gram) <- colnames(data_brs)[var_selected_gram]
    colnames(X_test_selected_s_gram) <- colnames(data_brs)[var_selected_gram]
    colnames(X_test_selected_p_gram) <- colnames(data_brs)[var_selected_gram]
  }
  ...
  for (s in 1:ncol(data_brs)){
    ...
    if (colnames(data_brs[s]) == colnames(X_train_selected_gram[t])){
  ...
  err_elast[b] <- sum(c(pred_elast_s,pred_elast_p)!=y_brs)/length(y_brs)
  pred <- c(pred_elast_s,pred_elast_p)
  mat[,b] <- pred
  tab <- table(y_brs,pred)
  ...
  err_covvsurf[b] <- sum(c(pred_rf_s,pred_rf_p)!=y_brs)/length(y_brs)
  pred_rf <- c(pred_rf_s,pred_rf_p)
  mat_rf[,b] <- pred_rf
  tab_rf <- table(y_brs,pred_rf)
  ...
  err_elast_synth[b] <- sum(c(pred_elast_s_synth,pred_elast_p_synth)!=y_brs)/length(y_brs)
  pred_elast_synth <- c(pred_elast_s_synth,pred_elast_p_synth)
  mat_elast_synth[,b] <- pred_elast_synth
  tab_elast_synth <- table(y_brs,pred_elast_synth)
  ...
  err_covvsurf_synth[b] <- sum(c(pred_rf_s_synth,pred_rf_p_synth)!=y_brs)/length(y_brs)
  pred_rf_synth <- c(pred_rf_s_synth,pred_rf_p_synth)
  mat_rf_synth[,b] <- pred_rf_synth
  tab_rf_synth <- table(y_brs,pred_rf_synth)
  ...
  err_elast_gram[b] <- sum(c(pred_elast_s_gram,pred_elast_p_gram)!=y_brs)/length(y_brs)
  pred_elast_gram <- c(pred_elast_s_gram,pred_elast_p_gram)
  mat_elast_gram[,b] <- pred_elast_gram
  tab_elast_gram <- table(y_brs,pred_elast_gram)
  ...
  err_covvsurf_gram[b] <- sum(c(pred_rf_s_gram,pred_rf_p_gram)!=y_brs)/length(y_brs)
  pred_rf_gram <- c(pred_rf_s_gram,pred_rf_p_gram)
  mat_rf_gram[,b] <- pred_rf_gram
  tab_rf_gram <- table(y_brs,pred_rf_gram)
...
vecteur_var_elast <- cbind(vecteur_var_elast,c('intercept',colnames(data_brs)))
```
...
vecteur_var_covs <- cbind(vecteur_var_covs,c('intercept',colnames(data_brs)))
```
...
```{r}
vect_patho_elast <- rep(0,dim(data_brs)[1])

for( l in 28:dim(data_brs)[1]){
  for(m in 1:B){
    if(mat[l,m]!=1){
...
```{r}
vect_patho_elast <- as.data.frame(vect_patho_elast[28:dim(data_brs)[1]])
vect_patho_elast$indiv <- c(28:dim(data_brs)[1])
...
```{r}
vect_patho_cov <- rep(0,dim(data_brs)[1])
...
```{r}
vect_patho_cov <- as.data.frame(vect_patho_cov[28:dim(data_brs)[1]])
vect_patho_cov$indiv <- c(28:dim(data_brs)[1])
...
vecteur_var_elast_gram <- cbind(vecteur_var_elast_gram,c('intercept',colnames(data_brs)))
```
...
vecteur_var_covs_gram <- cbind(vecteur_var_covs_gram,c('intercept',colnames(data_brs)))
```
...
vect_sain_elast_gram$indiv <- c(1:22)
...
vect_sain_cov_gram$indiv <- c(1:22)
...
```{r}
vect_patho_elast_gram <- rep(0,dim(data_brs)[1])
...
```{r}
vect_patho_elast_gram <- as.data.frame(vect_patho_elast_gram[28:dim(data_brs)[1]])
vect_patho_elast_gram$indiv <- c(28:dim(data_brs)[1])
...
```{r}
vect_patho_cov_gram <- rep(0,dim(data_brs)[1])
...
```{r}
vect_patho_cov_gram <- as.data.frame(vect_patho_cov_gram[28:dim(data_brs)[1]])
vect_patho_cov_gram$indiv <- c(28:dim(data_brs)[1])
...
```{r}
#kval <- c(2:19, seq(from = 20, to = 100, by = 5), seq(from = 101, to = ncol(data_brs), by = 300))
kval <- c(2:30)
nb_f <- 100
```

```{r}
mat_foret_brs <- matrix(NA,nb_f,length(kval))
```

```{r}
for(i in 1:nb_f){
  for(j in 1:length(kval)){
    cut <- cutreevar(tree_donnees_entieres_brs,j)
    var_synthe <- cut$scores
    rf <- randomForest(var_synthe,as.factor(y_brs),ntree = 300)
    err <- rf$err.rate[300,1]
    mat_foret_brs[i,j] <- err
  }
  print(i)
}
```
...
```{r}
# 1 for rows 
moyenne <- apply(mat_foret_brs, 2, mean)
```


```{r}
save(mat_foret_brs,file="/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/matrice_foret_brs.RData")
```

```{r}
load("matrice_foret_brs.RData")
```


```{r}
lim=c(0.05,0.35)
matplot(t(mat_foret_brs),type="l",ylim=lim,lty=2, xlab="kval", ylab="erreur OOB",main="Erreurs OOB sur 100 forêts pour 30 valeurs de clusters",las=2, col='black') 
...
```{r}
#comptage des variables utilisées
vecteur_var_elast_synth <- rep(0,11)
...
vecteur_var_elast_synth <- cbind(vecteur_var_elast_synth,c('intercept',colnames(var_synth_brs)))
```
...
```{r}
#comptage des variables utilisées
vecteur_var_covs_synth <- rep(0,11)
...
vecteur_var_covs_synth <- cbind(vecteur_var_covs_synth,c('intercept',colnames(var_synth_brs)))
```
...
```{r}
vect_sain_elast_synth <- rep(0,22)
...
vect_sain_elast_synth$indiv <- c(1:22)
...
vect_sain_cov_synth$indiv <- c(1:22)
...
```{r}
vect_patho_elast_synth <- rep(0,dim(data_brs)[1])
...
```{r}
vect_patho_elast_synth <- as.data.frame(vect_patho_elast_synth[28:dim(data_brs)[1]])
vect_patho_elast_synth$indiv <- c(28:dim(data_brs)[1])
...
```{r}
vect_patho_cov_synth <- rep(0,dim(data_brs)[1])
...
```{r}
vect_patho_cov_synth <- as.data.frame(vect_patho_cov_synth[28:dim(data_brs)[1]])
vect_patho_cov_synth$indiv <- c(28:dim(data_brs)[1])
...
```{r}
data_synth <- data.frame(y_brs,matrice_synth_10clusters)
```
...
#cercle des corrélations axes 1 et 2
#select.var = list(cos2=0.5),
brsz_pca_var(acp_synth,axes = c(1,2), col.var="contrib",  label="none") + labs(title="Cercle des corrélations sur les données synthétiques")+theme(plot.title =element_text(hjust = 0.5),plot.caption =element_text(hjust = 0.5))
```


```{r}
#plan factoriel axes 1 et 2
brsz_pca_ind(acp_synth,axes = c(1,2), habillage = 1, mean.point = FALSE)+ labs(title="Graphique des individus sur les données synthétiques")+theme(plot.title =element_text(hjust = 0.5),plot.caption =element_text(hjust = 0.5))
```
