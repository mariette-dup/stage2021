...
nouveau code après récupération des résulats 
...
Dans cette base, les 27 premières lignes sont des patients contrôles et les 23 patients suivants sont pathologiques (sans les NA). 

```{r}
data_fvi <- read.csv("/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/FVI_nom_new.csv" , sep=";",header = TRUE)
#data_fvi <- read.csv("FVI_nom.csv" , sep=";",header = TRUE)
```
...
```{r}
indNA <- which(is.na(data_fvi),arr.ind=TRUE)[,1]
data_fvi <- data_fvi[-indNA,]
```

```{r}
y_fvi <- rep(NA, nrow(data_fvi))

for (i in 1:nrow(data_fvi)){
    if (i < 28) {
        y_fvi[i] <- 0
    }
    else {
...
```{r}
for(i in 1:1680){
  colnames(data_fvi)[i] <- paste(Name_desc[1,i],Name_desc[2,i],Name_desc[3,i],Name_desc[4,i],sep = "_")
}
```

```{r}
side <- as.data.frame(setNames(replicate(1680,numeric(0), simplify = F),colnames(data_fvi) ))
side [1,] <- Name_desc[5,]
```
...
```{r}
#base pour l'ACP
data_f <- data.frame(y_fvi,data_fvi)
```
...
```{r}
et_f <- sort(apply(data_fvi,2,sd), decreasing = TRUE)
```


```{r}
barplot(et_f,xaxt="n",xlab="ordre des variables", ylab="écarts-types", main="Diagramme en barres des écarts-types
...
```{r}
#acp normée 
acp_norme <- PCA(data_f, scale.unit=TRUE, quali.sup =1, graph=F)
```
...
#cercle des corrélations axes 1 et 2
#select.var = list(cos2=0.5),
fviz_pca_var(acp_norme,axes = c(1,2), label="none") + labs(title="Cercle des corrélations sur les données d'origine")+theme(plot.title =element_text(hjust = 0.5),plot.caption =element_text(hjust = 0.5))
```
...
```{r}
sort(acp_norme$var$cos2[,1]+acp_norme$var$cos2[,2],decreasing = TRUE)[1:60]
```
...
```{r}
#tree_donnees_entieres_fvi <- hclustvar(X.quanti = data_fvi)
```

```{r}
#save(tree_donnees_entieres_fvi, file = "/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/tree_donnees_entieres_fvi.RData")
```

```{r}
load("/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/tree_donnees_entieres_fvi.RData")
```


```{r}
cut_VC_fvi <- cutreevar(tree_donnees_entieres_fvi,6)
var_synth_fvi <- cut_VC_fvi$scores
save(var_synth_fvi,file="/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/var_synth_fvi.RData")
load("var_synth_fvi.RData")
```
...
err_elast <- rep(NA,B) 
mat <- matrix(NA,dim(data_fvi)[1],B)
tvp <- rep(NA,B)
preci <- rep(NA,B)
...
mat_rf <- matrix(NA,dim(data_fvi)[1],B)
tvp_rf <- rep(NA,B)
preci_rf <- rep(NA,B)
...
mat_elast_synth <- matrix(NA,dim(data_fvi)[1],B)
tvp_elast_synth <- rep(NA,B)
preci_elast_synth <- rep(NA,B)
choix_var_elast_synth <- matrix(NA,n,7)
nb_var_elast_synth <- rep(NA,n)

err_covvsurf_synth <- rep(NA,B)
mat_rf_synth <- matrix(NA,dim(data_fvi)[1],B)
...
mat_elast_gram <- matrix(NA,dim(data_fvi)[1],B)
tvp_elast_gram <- rep(NA,B)
preci_elast_gram <- rep(NA,B)
choix_var_elast_gram <- matrix(NA,n,dim(data_fvi)[1]+1)
nb_var_elast_gram <- rep(NA,n)

err_covvsurf_gram <- rep(NA,B)
mat_rf_gram <- matrix(NA,dim(data_fvi)[1],B)
...
mat_gram_no <- matrix(NA,dim(data_fvi)[1],B)
...
sain_d <- data_fvi[1:27,]
patho_d <- data_fvi[28:dim(data_fvi)[1],]

sain_d_synth <- var_synth_fvi[1:27,]
patho_d_synth <- var_synth_fvi[28:dim(data_fvi)[1],]


y_sain <- y_fvi[1:27]
y_patho <- y_fvi[28:dim(data_fvi)[1]]
...
for (b in 1:B)
{
  ...
  patho <- sample(rep(1:n_folds, length.out = dim(data_fvi)[1]-27))
  ...
  for (alpha_ind in 1:length(alpha)) {
    ...
    err_alpha[alpha_ind] <- sum(c(pred_alpha_s,pred_alpha_p)!=y_fvi[c(test_i_s,test_i_p+27)])/length(y_fvi[c(test_i_s,test_i_p+27)])
  ...
  if(length(var_selected) == 1){
    ...
    colnames(X_train_selected) <- colnames(data_fvi)[var_selected]
    colnames(X_test_selected_s) <- colnames(data_fvi)[var_selected]
    colnames(X_test_selected_p) <- colnames(data_fvi)[var_selected]
  }
  ...
  for (s in 1:ncol(data_fvi)){
    for (t in 1:length(var_selected)){
    if (colnames(data_fvi[s]) == colnames(X_train_selected[t])){
  ...
  for (alpha_ind in 1:length(alpha)) {
    ...
    err_alpha_synth[alpha_ind] <- sum(c(pred_alpha_s_synth,pred_alpha_p_synth)!=y_fvi[c(test_i_s,test_i_p+27)])/length(y_fvi[c(test_i_s,test_i_p+27)])
  ...
  if(length(var_selected_synth) == 1){
    ...
    colnames(X_train_selected_synth) <- colnames(data_fvi)[var_selected_synth]
    colnames(X_test_selected_s_synth) <- colnames(data_fvi)[var_selected_synth]
    colnames(X_test_selected_p_synth) <- colnames(data_fvi)[var_selected_synth]
  }
  ...
  for (s in 1:ncol(data_fvi)){
    ...
    if (colnames(data_fvi[s]) == colnames(X_train_selected_synth[t])){
  ...
  # #données gram schmidt
  Ytrain_g <- t(t(Ytrain))
  index = gram(Xtrain,Ytrain_g,dim(data_fvi)[1])
  var_l_1se <- index[1:dim(data_fvi)[1]]
  c <- colnames(data_fvi)[var_l_1se]
  indices_1se <- which(colnames(data_fvi) %in% c)
  ...
  for (alpha_ind in 1:length(alpha)) {
    ...
    err_alpha_gram[alpha_ind] <- sum(c(pred_alpha_s_gram,pred_alpha_p_gram)!=y_fvi[c(test_i_s,test_i_p+27)])/length(y_fvi[c(test_i_s,test_i_p+27)])
  ...
  if(length(var_selected_gram) == 1){
    ...
    colnames(X_train_selected_gram) <- colnames(data_fvi)[var_selected_gram]
    colnames(X_test_selected_s_gram) <- colnames(data_fvi)[var_selected_gram]
    colnames(X_test_selected_p_gram) <- colnames(data_fvi)[var_selected_gram]
  }
  ...
  for (s in 1:ncol(data_fvi)){
    ...
    if (colnames(data_fvi[s]) == colnames(X_train_selected_gram[t])){
  ...
  err_elast[b] <- sum(c(pred_elast_s,pred_elast_p)!=y_fvi)/length(y_fvi)
  pred <- c(pred_elast_s,pred_elast_p)
  mat[,b] <- pred
  tab <- table(y_fvi,pred)
  ...
  err_covvsurf[b] <- sum(c(pred_rf_s,pred_rf_p)!=y_fvi)/length(y_fvi)
  pred_rf <- c(pred_rf_s,pred_rf_p)
  mat_rf[,b] <- pred_rf
  tab_rf <- table(y_fvi,pred_rf)
  ...
  err_elast_synth[b] <- sum(c(pred_elast_s_synth,pred_elast_p_synth)!=y_fvi)/length(y_fvi)
  pred_elast_synth <- c(pred_elast_s_synth,pred_elast_p_synth)
  mat_elast_synth[,b] <- pred_elast_synth
  tab_elast_synth <- table(y_fvi,pred_elast_synth)
  ...
  err_covvsurf_synth[b] <- sum(c(pred_rf_s_synth,pred_rf_p_synth)!=y_fvi)/length(y_fvi)
  pred_rf_synth <- c(pred_rf_s_synth,pred_rf_p_synth)
  mat_rf_synth[,b] <- pred_rf_synth
  tab_rf_synth <- table(y_fvi,pred_rf_synth)
  ...
  err_elast_gram[b] <- sum(c(pred_elast_s_gram,pred_elast_p_gram)!=y_fvi)/length(y_fvi)
  pred_elast_gram <- c(pred_elast_s_gram,pred_elast_p_gram)
  mat_elast_gram[,b] <- pred_elast_gram
  tab_elast_gram <- table(y_fvi,pred_elast_gram)
  ...
  err_covvsurf_gram[b] <- sum(c(pred_rf_s_gram,pred_rf_p_gram)!=y_fvi)/length(y_fvi)
  pred_rf_gram <- c(pred_rf_s_gram,pred_rf_p_gram)
  mat_rf_gram[,b] <- pred_rf_gram
  tab_rf_gram <- table(y_fvi,pred_rf_gram)
...
vecteur_var_elast <- cbind(vecteur_var_elast,c('intercept',colnames(data_fvi)))
```
...
vecteur_var_covs <- cbind(vecteur_var_covs,c('intercept',colnames(data_fvi)))
```
...
```{r}
vect_patho_elast <- rep(0,dim(data_fvi)[1])

for( l in 28:dim(data_fvi)[1]){
  for(m in 1:B){
    if(mat[l,m]!=1){
...
```{r}
vect_patho_elast <- as.data.frame(vect_patho_elast[28:dim(data_fvi)[1]])
vect_patho_elast$indiv <- c(28:dim(data_fvi)[1])
...
```{r}
vect_patho_cov <- rep(0,dim(data_fvi)[1])
...
```{r}
vect_patho_cov <- as.data.frame(vect_patho_cov[28:dim(data_fvi)[1]])
vect_patho_cov$indiv <- c(28:dim(data_fvi)[1])
...
vecteur_var_elast_gram <- cbind(vecteur_var_elast_gram,c('intercept',colnames(data_fvi)))
```
...
vecteur_var_covs_gram <- cbind(vecteur_var_covs_gram,c('intercept',colnames(data_fvi)))
```
...
```{r}
vect_patho_elast_gram <- rep(0,dim(data_fvi)[1])
...
```{r}
vect_patho_elast_gram <- as.data.frame(vect_patho_elast_gram[28:dim(data_fvi)[1]])
vect_patho_elast_gram$indiv <- c(28:dim(data_fvi)[1])
...
```{r}
vect_patho_cov_gram <- rep(0,dim(data_fvi)[1])
...
```{r}
vect_patho_cov_gram <- as.data.frame(vect_patho_cov_gram[28:dim(data_fvi)[1]])
vect_patho_cov_gram$indiv <- c(28:dim(data_fvi)[1])
...
```{r}
#kval <- c(2:19, seq(from = 20, to = 100, by = 5), seq(from = 101, to = ncol(data_fvi), by = 300))
kval <- c(2:30)
nb_f <- 100
```

```{r}
mat_foret_fvi <- matrix(NA,nb_f,length(kval))
```

```{r}
for(i in 1:nb_f){
  for(j in 1:length(kval)){
    cut <- cutreevar(tree_donnees_entieres_fvi,j)
    var_synthe <- cut$scores
    rf <- randomForest(var_synthe,as.factor(y_fvi),ntree = 300)
    err <- rf$err.rate[300,1]
    mat_foret_fvi[i,j] <- err
  }
  print(i)
}
```
...
```{r}
# 1 for rows 
moyenne <- apply(mat_foret_fvi, 2, mean)
```


```{r}
save(mat_foret_fvi,file="/Volumes/mariette.dupuy/GitHub/IHU-LIRYC/matrice_foret_fvi.RData")
```

```{r}
load("matrice_foret_fvi.RData")
```


```{r}
lim=c(0.05,0.35)
matplot(t(mat_foret_fvi),type="l",ylim=lim,lty=2, xlab="kval", ylab="erreur OOB",main="Erreurs OOB sur 100 forêts pour 30 valeurs de clusters",las=2, col='black') 
...
```{r}
#comptage des variables utilisées
vecteur_var_elast_synth <- rep(0,7)
...
vecteur_var_elast_synth <- cbind(vecteur_var_elast_synth,c('intercept',colnames(var_synth_fvi)))
```
...
```{r}
barplot(tab_elast_synth[order(tab_elast_synth[2:11,1],decreasing=TRUE),][2:11,1],ylab="nombre de fois que la variable est sélectionnée", space =1)
end_point = 0.5 + nrow(tab_elast_synth[2:11,]) + nrow(tab_elast_synth[2:11, ]) - 1
text(seq(1.5, end_point, by = 2), par("usr")[3]-0.25, 
     srt = 60, adj = 1, xpd = TRUE,
     labels = tab_elast_synth[order(tab_elast_synth[2:11,1],decreasing=TRUE),][2:11,2], cex = 0.8)
```


```{r}
#comptage des variables utilisées
vecteur_var_covs_synth <- rep(0,7)
...
vecteur_var_covs_synth <- cbind(vecteur_var_covs_synth,c('intercept',colnames(var_synth_fvi)))
```
...
```{r}
barplot(tab_cov_synth[order(tab_cov_synth[2:11,1],decreasing=TRUE),][2:11,1], ylab="nombre de fois que la variable est sélectionnée", space =1)
end_point = 0.5 + nrow(tab_cov_synth[2:11,]) + nrow(tab_cov_synth[2:11, ]) - 1
text(seq(1.5, end_point, by = 2), par("usr")[3]-0.25, 
     srt = 60, adj = 1, xpd = TRUE,
     labels = tab_cov_synth[order(tab_cov_synth[2:11,1],decreasing=TRUE),][2:11,2], cex = 0.8)
```
...
```{r}
vect_patho_elast_synth <- rep(0,dim(data_fvi)[1])
...
```{r}
vect_patho_elast_synth <- as.data.frame(vect_patho_elast_synth[28:dim(data_fvi)[1]])
vect_patho_elast_synth$indiv <- c(28:dim(data_fvi)[1])
...
```{r}
vect_patho_cov_synth <- rep(0,dim(data_fvi)[1])
...
```{r}
vect_patho_cov_synth <- as.data.frame(vect_patho_cov_synth[28:dim(data_fvi)[1]])
vect_patho_cov_synth$indiv <- c(28:dim(data_fvi)[1])
...
```{r}
boxplot(data.frame(elasticnet=err_elast, VSURF=err_covvsurf, elast_gram=err_elast_gram, VSURF_gram=err_covvsurf_gram, elast_synth=err_elast_synth, VSURF_synth=err_covvsurf_synth), ylab="Taux d'erreur",las=2,cex.axis=0.7,col = c("#0C6C83","#0C6C83","#838322","#838322","#70144A","#70144A"))
```


```{r}
boxplot(data.frame(elasticnet=nb_var_elast, VSURF=nb_var_covs, elast_gram=nb_var_elast_gram, VSURF_gram=nb_var_covs_gram, elast_synth=nb_var_elast_synth, VSURF_synth=nb_var_covs_synth), ylab="Nombre de variables sélectionnées",las=2,cex.axis=0.7,col = c("#0C6C83","#0C6C83","#838322","#838322","#70144A","#70144A"))
```


```{r}
boxplot(data.frame(elasticnet=tvp, VSURF=tvp_rf, elast_gram=tvp_elast_gram, VSURF_gram=tvp_rf_gram, elast_synth=tvp_elast_synth, VSURF_synth=tvp_rf_synth),las=2,cex.axis=0.7, ylab="sensibilité",col = c("#0C6C83","#0C6C83","#838322","#838322","#70144A","#70144A"))
```
...
```{r}
boxplot(data.frame(elasticnet=preci, VSURF=preci_rf, elast_gram=preci_elast_gram, VSURF_gram=preci_rf_gram, elast_synth=preci_elast_synth, VSURF_synth=preci_rf_synth), ylab="precision",las=2,cex.axis=0.7,col = c("#0C6C83","#0C6C83","#838322","#838322","#70144A","#70144A"))
```
...
```{r}
data_synth <- data.frame(y_fvi,matrice_synth_10clusters)
```
